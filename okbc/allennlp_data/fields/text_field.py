...
class TextField(SequenceField[TextFieldTensors]):
    ...
    to tokenize raw strings using a :class:`~allennlp_data.tokenizers.tokenizer.Tokenizer`.

    Because string tokens can be represented as indexed arrays in a number of ways, we also take a
    dictionary of :class:`~allennlp_data.token_indexers.token_indexer.TokenIndexer`
