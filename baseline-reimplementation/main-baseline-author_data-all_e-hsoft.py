...
from models import rotatELSTM, complexLSTM, complexLSTM_2, complexLSTM_2_all_e, complexLSTM_2_all_e_hsoftmax
...
def main(args):
    ...
    model = complexLSTM_2_all_e_hsoftmax(len(etoken_map)+1,len(rtoken_map)+1, len(entity_mentions), args.embedding_dim, {}, initial_token_embedding =args.initial_token_embedding, entity_tokens = etokens, relation_tokens = rtokens, lstm_dropout=args.lstm_dropout)
    ...
    if(args.do_train):
        ...
        for epoch in tqdm(range(0,args.num_train_epochs), desc="epoch"):
            ...
            for batch in tqdm(train_loader, desc="Train dataloader"):
                ...
                inputs_new = [None,None]
                if inputs[0]!=None:
                    inputs_new[0] = [None,None]
                    inputs_new[0][0] = inputs[0][0] - 2
                    inputs_new[0][1] = inputs[0][1] - 2
                if inputs[1]!=None:
                    inputs_new[1] = [None,None]
                    inputs_new[1][0] = inputs[1][0] - 2
                    inputs_new[1][1] = inputs[1][1] - 2
                batch_shared_entities -= 2
                # this_head, this_tail = model.get_all_tree_nodes_debug(inputs_new, batch_shared_entities, labels)
                # head.extend(this_head)
                # tail.extend(this_tail)
                # ct+=1
                # if ct==1000:
                #     import pdb
                #     pdb.set_trace()
                ...
                head_inputs, tree_nodes_head, head_mask, tail_inputs, tree_nodes_tail, tail_mask = model.get_all_tree_nodes(inputs_new, batch_shared_entities, labels)
                loss = torch.tensor(0,device=device)
                for mode,model_inputs in zip(["head","tail"],[head_inputs,tail_inputs]):
                    if model_inputs==None:
                        continue
                    if mode=="head":
                        batch_e2_indices = model_inputs[1]
                        batch_e2_indices = batch_e2_indices.where(batch_e2_indices!=-2,torch.tensor(len(entity_mentions)-2,dtype=torch.int32))
                        batch_r_indices  = model_inputs[0]
                        ...
                        model_output = model.complex_score_e2_r_with_given_ementions(e2_real_lstm,e2_img_lstm,r_real_lstm,r_img_lstm,tree_nodes_head[0],tree_nodes_head[1])
                        loss = loss - (logsigmoid(model_output*head_mask)).mean()
                    else:
                        batch_e1_indices = model_inputs[0]
                        batch_e1_indices = batch_e1_indices.where(batch_e1_indices!=-2,torch.tensor(len(entity_mentions)-2,dtype=torch.int32))
                        batch_r_indices  = model_inputs[1]
                        batch_r_indices = batch_r_indices.where(batch_r_indices!=-2,torch.tensor(len(relation_mentions)-2,dtype=torch.int32))
                        ...
                        model_output = model.complex_score_e1_r_with_given_ementions(e1_real_lstm,e1_img_lstm,r_real_lstm,r_img_lstm,tree_nodes_tail[0],tree_nodes_tail[1])
                        loss = loss - (logsigmoid(model_output*tail_mask)).mean()
                        # # going one by one, takes maximum time
                        # for i in tqdm(range(batch_r_indices.shape[0])):
                        #   e_r = e1_real_lstm[i]
                        #   e_i = e1_img_lstm[i]
                        #   r_r = r_real_lstm[i]
                        #   r_i = r_img_lstm[i]
                        #   correct_labels = batch_shared_entities[labels[global_ct].nonzero().squeeze()]
                        #   global_ct += 1
                        #   if correct_labels.nelement()!=0:
                        #       for target in correct_labels:
                        #           loss = loss + model.get_loss(e_r,e_i,r_r,r_i,target.item(),mode)
    ...
    if args.do_eval:
        # Techniques tried for testing (to test a single e1,r,?):
        #   1. Create tensor of #entities(targets) x tree_node_indices -> Get tree node embeddings -> CUDA ERROR (turns out i cant save more than 2.4mil x 5 x 512 tensor on CUDA. Right now max_tree_depth is 42) [cons: repeated computation | pros: massive gpu parallelization]
        #   2. Get the embeddings of all tree nodes(aka X) and score it with query e1,r. Now for each entity mention(target), get indices of its nodes and selectively add just their score from X (TAKES 2 MINUTES FOR A SINGLE QUERY)
        #   3. Do a proper tree traversal by starting from root and going to each leaf(target) while multiplying that node's embedding with query e1,r. Do proper +,- based on left,right. When reach leaf(target), store the current score (TAKES 10 MINUTES FOR A SINGLE QUERY)
