...
from models import rotatELSTM, complexLSTM, complexLSTM_2, complexLSTM_2_all_e
...
def main(args):
 ...
 #        )
	...
	# print(len(etoken_map)+1)
	# print(len(rtoken_map)+1)
	# print(len(entity_mentions))
	# print(args.embedding_dim)
	# print(args.lstm_dropout)
	# sys.exit()

	model = complexLSTM_2_all_e(len(etoken_map)+1,len(rtoken_map)+1, len(entity_mentions), args.embedding_dim, initial_token_embedding =args.initial_token_embedding, entity_tokens = etokens, relation_tokens = rtokens, lstm_dropout=args.lstm_dropout)
			...
			for batch in tqdm(train_loader, desc="Train dataloader"):
				...
				for mode,model_inputs in zip(["head","tail"],inputs):
					...
					if mode=="head":
						...
						r_real_lstm, r_img_lstm   = model.get_mention_embedding(train_r_mention_tensor,1,train_r_lengths)
						e2_real_lstm, e2_img_lstm = model.get_atomic_entity_embeddings(batch_e2_indices.squeeze(1).long().cuda())
	...
	if args.do_eval:
		...
		ementions_real, ementions_img = model.get_atomic_entity_embeddings(torch.tensor(range(len(entity_mentions)), device=device, dtype=torch.long))
		########################################################################
		if "olpbench" in args.data_dir:
